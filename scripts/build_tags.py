...
def merge_entry(works: dict[str, Any], slug: str, tags: Any, source: str, overwrite_existing: bool = False) -> bool:
    if not slug: return False
    existed = isinstance(works.get(slug), dict)
    ...
    seed = [*entry.get("tags", []), *normalize_tags(tags)] if existed else normalize_tags(tags)
    entry["tags"] = normalize_tags(seed)
...
def legacy_public_false_slugs(data_dir: Path) -> set[str]:
    found: set[str] = set()
    for name in ["fetch.json", "rotunda.json"]:
        ...
        for item in rows:
            if isinstance(item, dict) and item.get("public") is False and item.get("slug"):
                found.add(str(item["slug"]))
    for path in sorted((data_dir / "works").rglob("*.json")):
        item = load_json(path, {})
        if isinstance(item, dict) and item.get("public") is False:
            found.add(str(item.get("slug") or path.stem))
    return found

def migrate_legacy_public_false(data_dir: Path, dry_run: bool = False) -> int:
    ...
    before.update(legacy_public_false_slugs(data_dir))
    policy["omit_works"] = sorted(before)
    rotunda["public_rotunda"] = policy
    changed = len(before) - len(set(str(x) for x in omit))
    if changed and not dry_run:
        atomic_write(rotunda_path, rotunda)
    return changed


def catalog_from_local(data_dir: Path, overwrite_existing: bool = False) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    for path in sorted((data_dir/"works").glob("*.json")):
        ...
        item = load_json(path, {})
        if isinstance(item, dict) and merge_entry(works, str(item.get("slug") or path.stem), item.get("tags"), "manifest", overwrite_existing):
            counts["changed_entries"] += 1
    for name, source in [("fetch.json","fetch"),("rotunda.json","rotunda")]:
        data = load_json(data_dir/name, {})
        rows = data.get("works", []) if isinstance(data, dict) else data if isinstance(data, list) else []
        for item in rows:
            if isinstance(item, dict) and merge_entry(works, str(item.get("slug") or ""), item.get("tags"), source, overwrite_existing):
                counts["changed_entries"] += 1
...
def merge_r2(catalog: dict[str, Any], remote: str, workers: int, overwrite_existing: bool = False) -> dict[str, int]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
        for _name, data, err in ex.map(rclone_cat, files):
            ...
            if slug and merge_entry(catalog.setdefault("works",{}), slug, tags, "r2-details", overwrite_existing=overwrite_existing): counts["imported"] += 1
...
def main(argv=None):
    ...
    ap.add_argument("--from-r2-details", action="store_true"); ap.add_argument("--overwrite-existing-tags", action="store_true", help="Opt in to merging stale mirrors into existing canonical tags.json entries."); ap.add_argument("--remote", default="animeplex.lol:extended/works"); ap.add_argument("--workers", type=int, default=8)
    args=ap.parse_args(argv); data_dir=(repo_root()/args.data_dir).resolve()
    catalog, counts = catalog_from_local(data_dir, args.overwrite_existing_tags)
    counts["legacy_public_false_migrated"] = migrate_legacy_public_false(data_dir, dry_run=args.dry_run)
    if args.from_r2_details: counts.update({f"r2_{k}":v for k,v in merge_r2(catalog,args.remote,args.workers,args.overwrite_existing_tags).items()})
